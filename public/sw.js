...
const VERSION = "1.2.69"; // App version
...
self.addEventListener("message", (event) => {
  ...
  } else if (event.data && event.data.type === "APP_UPDATE_COMPLETE") {
    console.log("SW: App update completed, notifying clients");
    // Broadcast to all clients that the app update is complete
  ...
  event.waitUntil(
    caches
      ...
      .then(() => {
        ...
        // Don't send automatic update notifications here
        // Only send them when explicitly requested via message
      })
...
// Remove duplicate push/notification handlers - they are defined below
...
async function syncExchangeRatesInBackground() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Don't show automatic notifications for background sync
      // Only notify through messages to avoid unwanted notifications
